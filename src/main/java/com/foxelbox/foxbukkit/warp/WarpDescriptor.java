 ...
 */
package com.foxelbox.foxbukkit.warp;
...
public class WarpDescriptor {
	...
	public WarpDescriptor(FoxBukkit plugin, UUID ownerName, String name, Location location) {
		this.plugin = plugin;
	...
	public WarpDescriptor(FoxBukkit plugin, String name, Map<String, List<String>> section) {
		this.plugin = plugin;
		this.name = name;
		load(section);
	}
	...
	public int checkAccess(CommandSender commandSender) {
		...
		if (playerLevel > ownerLevel && commandSender.hasPermission("foxbukkit.warp.override"))
			return 3;
