...
public abstract class ICommand {
	...
	protected FoxBukkit plugin;
	protected PlayerHelper playerHelper;

	protected ICommand() {
		this(FoxBukkit.instance.commandSystem);
	}
	...
	protected String parseFlags(String argStr) throws FoxBukkitCommandException {
	...
	protected String[] parseFlags(String[] args) throws FoxBukkitCommandException {
		...
		while (nextArg < args.length) {
			...
			for (int i = 1; i < arg.length(); ++i) {
				...
				if (flagType == null)
					throw new FoxBukkitCommandException("Invalid flag '"+flagName+"' specified.");
				...
				case STRING:
					...
					if (nextArg >= args.length)
						throw new FoxBukkitCommandException("No value specified for "+flagName+" flag.");
	...
	public static Player asPlayer(CommandSender commandSender) throws FoxBukkitCommandException {
		if (!(commandSender instanceof Player))
			throw new FoxBukkitCommandException("This command can only be run as a player.");

		return (Player) commandSender;
	}

	public static Location getCommandSenderLocation(CommandSender commandSender, boolean elevated) throws FoxBukkitCommandException {
		final Location location = getCommandSenderLocation(commandSender, elevated, null);
		if (location == null)
			throw new FoxBukkitCommandException("This command can only be run as a player or a command block.");
	...
	public static CraftPlayer asCraftPlayer(CommandSender commandSender) throws FoxBukkitCommandException {
		if (!(commandSender instanceof Player))
			throw new FoxBukkitCommandException("This command can only be run as a player.");

		if (!(commandSender instanceof CraftPlayer))
			throw new FoxBukkitCommandException("This command can only be run on CraftBukkit.");

		return (CraftPlayer) commandSender;
	}

	public static EntityPlayer asNotchPlayer(CommandSender commandSender) throws FoxBukkitCommandException {
		return asCraftPlayer(commandSender).getHandle();
	}

	public static EntityPlayer asNotchPlayer(CommandSender commandSender, EntityPlayer defaultValue) throws FoxBukkitCommandException {
		if (!(commandSender instanceof CraftPlayer))
			return defaultValue;
