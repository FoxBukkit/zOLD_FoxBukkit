...
import de.doridian.foxbukkit.main.offlinebukkit.OfflinePlayer;
import de.doridian.foxbukkit.main.util.MultiplePlayersFoundException;
import de.doridian.foxbukkit.main.util.PlayerNotFoundException;
...
import de.doridian.foxbukkit.warp.WarpException;
...
public class PlayerHelper extends StateContainer {
	...
	public PlayerHelper(FoxBukkit plug) {
		plugin = plug;
	}
	...
	public Map<String,String> playerNameToUUID = FoxBukkit.instance.redisManager.createCachedRedisMap("playerNameToUUID");
	public Map<String,String> playerUUIDToName = FoxBukkit.instance.redisManager.createCachedRedisMap("playerUUIDToName");
	...
	public Location getPlayerHomePosition(Player ply, String posName) throws FoxBukkitCommandException {
		...
		if(playerhomepos.containsKey(uuid)) {
			...
			if(playersPositions.containsKey(posName)) {
				...
				throw new FoxBukkitCommandException("Home position with that name was not found");
			}
		} else {
			return getPlayerSpawnPosition(ply);
		}
	}
	public void setPlayerHomePosition(Player ply, String posName, Location pos) throws FoxBukkitCommandException {
		UUID uuid = ply.getUniqueId();
		posName = posName.toLowerCase();
		...
		if(pos == null) {
			...
			throw new FoxBukkitCommandException("You cannot set more home positions");
		}
	...
	public void loadPlayerHomePositions() {
		playerhomepos.clear();
		try {
			BufferedReader stream = new BufferedReader(FoxBukkit.instance.configuration.makeReader("player-homepositions.txt"));
	...
	public void savePlayerHomePositions() {
		try {
			final BufferedWriter stream = new BufferedWriter(FoxBukkit.instance.configuration.makeWriter("player-homepositions.txt"));
	...
	/**
	 * Broadcasts a message to all players with the given permission, prefixed with [FB] in purple.
	 *
	...
	/**
	 * Broadcasts a message to all players with the given permission, prefixed with [FB] in the given color.
	 *
	 * @param message The message to send
	 * @param permission The permission required to receive the message
	...
	public static String getPlayerRank(UUID uuid) {
		final String rank = FoxBukkitPermissionHandler.instance.getGroup(uuid);
		if (rank == null)
			return "guest";
	...
	public Scoreboard createFBScoreboard() {
	...
	//Permission levels
	public Map<String,String> ranklevels = FoxBukkit.instance.redisManager.createCachedRedisMap("ranklevels");
	...
	//Tags
	private final Map<String,String> rankTags = FoxBukkit.instance.redisManager.createCachedRedisMap("ranktags");
	private final Map<String,String> playerTags = FoxBukkit.instance.redisManager.createCachedRedisMap("playerTags");
	...
	public void loadPortPermissions() {
		...
		try {
			BufferedReader stream = new BufferedReader(FoxBukkit.instance.configuration.makeReader("player-notp.txt"));
			String line;
			while((line = stream.readLine()) != null) {
		...
		try {
			BufferedReader stream = new BufferedReader(FoxBukkit.instance.configuration.makeReader("player-nosummon.txt"));
			String line;
			while((line = stream.readLine()) != null) {
	...
	public void savePortPermissions() {
		try {
			BufferedWriter stream = new BufferedWriter(FoxBukkit.instance.configuration.makeWriter("player-notp.txt"));
		...
		try {
			BufferedWriter stream = new BufferedWriter(FoxBukkit.instance.configuration.makeWriter("player-nosummon.txt"));
	...
	public void loadAutoexecs() {
		autoexecs.clear();
		try {
			BufferedReader stream = new BufferedReader(FoxBukkit.instance.configuration.makeReader("autoexecs.txt"));
	...
	public void saveAutoexecs() {
		try {
			BufferedWriter stream = new BufferedWriter(FoxBukkit.instance.configuration.makeWriter("autoexecs.txt"));
