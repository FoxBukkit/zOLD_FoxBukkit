 ...
 */
package de.doridian.foxbukkit.foxpoints;
...
public class FBBank extends StateContainer {
	Map<UUID, FBAccount> accounts = new HashMap<>();

	private FBAccount getAccount(UUID playerName) {
		FBAccount account = accounts.get(playerName);

		if (account == null) {
			accounts.put(playerName, account = new FBAccount(this));
		}
	...
	public void load() {
		...
		for (Entry<String, List<Map<String, List<String>>>> entry : sections.entrySet()) {
			...
			accounts.put(playerName, new FBAccount(this, namesakes.get(0)));
	...
	public void save() {
		...
		for (Entry<UUID, FBAccount> entry : accounts.entrySet()) {
			FBAccount account = entry.getValue();
