 ...
 */
package de.doridian.foxbukkit.bans.listeners;

import de.doridian.foxbukkit.bans.Ban;
import de.doridian.foxbukkit.bans.BanPlayer;
import de.doridian.foxbukkit.bans.BanResolver;
import de.doridian.foxbukkit.bans.LockDownMode;
import de.doridian.foxbukkit.core.FoxBukkit;
import de.doridian.foxbukkit.core.util.MessageHelper;
import de.doridian.foxbukkit.core.util.PermissionPredicate;
...
public class BansPlayerListener extends BaseListener {
	private final boolean IS_BAN_AND_ALT_CHECK_ENABLED = FoxBukkit.instance.configuration.getValue("enable-ban-and-alt-check", "true").equalsIgnoreCase("true");
	...
	public void onPlayerPreLogin(AsyncPlayerPreLoginEvent event) {
		...
		if(ban != null)
			event.disallow(Result.KICK_BANNED, "[FB] Banned: " + ban.getReason());
	}
	...
	public void onPlayerLogin(PlayerLoginEvent event) {
		...
		if (playerHelper.isGuest(player)) {
			switch (plugin.bans.lockdownMode) {
				...
				case KICK:
					event.disallow(PlayerLoginEvent.Result.KICK_BANNED, "[FB] Sorry, we're closed for guests right now");
				/* FALL-THROUGH */
		...
		new Thread() {
			public void run() {
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(FoxBukkit.instance, new Runnable() {
					@Override
					public void run() {
					MessageHelper.sendColoredServerMessage("light_purple", new PermissionPredicate("foxbukkit.opchat"), message);
					}
				});
			}
		}.start();
