...
@Usage("[<flags>][<command>]")
@Permission("yiffbukkitsplit.transmute.shapeaction")
@BooleanFlags("el")
@StringFlags("i")
public class ShapeActionCommand extends ICommand {
	...
	public void Run(Player ply, String[] args, String argStr) throws YiffBukkitCommandException {
		...
		if (booleanFlags.contains('e')) {
			if (!plugin.permissionHandler.has(ply, "yiffbukkitsplit.transmute.shapeaction.others"))
			...
			playerHelper.addToolMapping(ply, toolType, new ToolBind(shapeAction, ply) {
				...
				public void run(PlayerInteractEntityEvent event) throws YiffBukkitCommandException {
					...
					if (!plugin.permissionHandler.has(player, "yiffbukkitsplit.transmute.shapeaction.others"))
						throw new PermissionDeniedException();
